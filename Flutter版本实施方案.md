...
> 文档状态：阶段0～5交付分别经PR #1/#2/#6/#8/#11等远端CI验证并合并；2026-09-21阶段5（登录、收藏、主题和我的）经PR #11/#12/#13合并。冻结不代表视觉验收、真实账号验收、真实进程恢复或发布检查已经实现。
...
2026-09-25主题独立审核修正回填：`ThemeController`改为串行写入并合并保存中产生的新选择，保证最新用户意图最终持久化；主题配色与模式改为单个版本化SharedPreferences值原子保存，并兼容阶段5初版双键读取。最终写失败回滚最后持久化值，读取失败和每次配色/模式保存失败均给出可见反馈。主题页暗色选中态和色卡使用当前亮度ColorScheme，`wanTheme`补齐Android冻结色板的inverse/fixed/surfaceDim/surfaceBright等角色。主题专项19/19、全量128/128、静态/架构/敏感数据正反夹具、阶段1～5结构检查、代码生成及双端Debug构建通过；当前设备列表只有macOS/Chrome，最终修正版Android/iOS阶段5集成未执行，远端CI、200%全组合、逐页视觉/无障碍和阶段6门禁仍待验证。

2026-09-26架构债务清理回填：Profile、收藏、主题设置、阅读历史及阅读器历史写入逐项迁移为View→ViewModel→Repository/Controller，原Profile三页的5条精确临时架构白名单全部删除；同时补上Repository Provider声明扫描，阻止View通过`core/reader`等中间Provider绕过Data导入规则。新增违规失败与合法ViewModel通过夹具及4项ViewModel行为测试。最终格式210文件无变化、全量134/134、静态分析、101源码/9架构夹具、267文本/4敏感数据夹具、阶段1～5结构、代码/图标生成及双端Debug构建通过；当前只有macOS/Chrome，最终修正版双端集成及远端CI未执行。

2026-09-26扩大架构审查回填：阅读器收藏业务从`app/router`迁入Reader ViewModel，路由仅保留导航，孤立的`sessionStoreProvider`及内存存储删除；Reader View解除对Navigation反向依赖，Home可见性由ViewModel发布UiState，AuthRepository契约与Collection模型去除Flutter foundation。架构门禁扩展为Router/Feature Navigation不得访问Data、组合根或Repository Provider，View不得反向依赖Navigation，Repository契约/Model不得依赖Flutter，Feature不得直接引入Dio/Drift/安全存储/Preferences。最终102源码/16正反夹具、212文件格式、`flutter analyze`与Analysis Server零诊断、269文本/4敏感数据夹具、阶段1～5结构、144/144 Unit/Widget、代码/图标生成及47个文档链接通过；MI 9/Android 11/API30真机阶段4 9/9、阶段5 2/2，Android Debug APK构建成功。当前无iOS目标，最终iOS集成/Simulator Debug构建及远端CI未执行；阶段4入口返回断言不替代真实系统边缘手势专项。
