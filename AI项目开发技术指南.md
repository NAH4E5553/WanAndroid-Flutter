...
当前阶段为“阶段5：登录、收藏、主题和我的”，交付已合并（PR #11/#12/#13）；阶段2～4的生产只读链路、专题、阅读器与历史均已交付。本阶段落地了主题持久化与失败回滚、会话权威（SessionStore/串行提交协调器/会话拦截器）、登录恢复/退出/登录门禁、完整收藏契约（writeVersion/collectionSession/reconcile/setCollected）与我的/收藏/登录/主题设置界面。

阶段0冻结和阶段1～4完成均不代表后续业务已经实现。阶段5不得：

- 在自动化测试中接入真实账号或真实收藏写操作（真实账号验收与Fake自动化分开记录，属阶段6）。
- 提前实现注册、找回密码、离线缓存或发布能力。
- 将阶段0原型证据描述成生产集成已完成。
...
[ ] 已检查现有修改并保护用户工作
[ ] 新功能已从 docs/开发任务模板.md 生成开工单；小修复已记录不适用理由
[ ] 已定位 Android 基线来源、调用方和测试
[ ] 已确认现有 Flutter 封装可否复用
[ ] 已列出行为目标、非目标、唯一事实源、身份语义和高风险竞争场景
