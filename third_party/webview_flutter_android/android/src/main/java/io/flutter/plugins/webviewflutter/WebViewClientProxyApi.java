...
public class WebViewClientProxyApi extends PigeonApiWebViewClient {
  /// Local-patch channel reporting WebView renderer death; the payload is
  /// `[String? url, bool didCrash]` (StandardMessageCodec).
  public static final String RENDER_PROCESS_GONE_CHANNEL =
  ...
  public static class WebViewClientImpl extends WebViewClient {
    ...
    // WanAndroid Flutter local patch (stage 4, 2026-09-21): follow the official
    // onRenderProcessGone contract (developer.android.com, "Handle WebView
    // termination"). Returning true keeps this app alive when the sandboxed
    // renderer is killed (OOM, WebView provider update, or crash); the message
    // reports the event to Dart, which maps it to
    // WebResourceErrorType.webContentProcessTerminated and rebuilds the
    // WebView. API 24/25 never invoke this callback and keep the default
    // whole-app kill behavior.
    @Override
    @TargetApi(Build.VERSION_CODES.O)
    public boolean onRenderProcessGone(
        @NonNull WebView view, @NonNull RenderProcessGoneDetail detail) {
      final ArrayList<Object> message = new ArrayList<>();
      message.add(view.getUrl());
      message.add(detail.didCrash());
      new BasicMessageChannel<>(
              api.getPigeonRegistrar().getBinaryMessenger(),
              RENDER_PROCESS_GONE_CHANNEL,
