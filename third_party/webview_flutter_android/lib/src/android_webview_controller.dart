...
class AndroidWebResourceError extends WebResourceError {
  ...
  static WebResourceErrorType? _errorCodeToErrorType(int errorCode) {
    switch (errorCode) {
      ...
      case WebViewClientConstants.errorWebContentProcessTerminated:
...
class AndroidNavigationDelegate extends PlatformNavigationDelegate {
  ...
  AndroidNavigationDelegate(PlatformNavigationDelegateCreationParams params)
    ...
    // WanAndroid Flutter local patch (stage 4, 2026-09-21): receive renderer
    // death reports from the local `onRenderProcessGone` override in
    // WebViewClientProxyApi and surface them as a main-frame
    // webContentProcessTerminated error, matching the iOS path. The payload is
    // `[String? url, bool didCrash]`.
    BasicMessageChannel<Object?>(
      'dev.flutter.local_patch.webview_flutter_android/WebViewClient.onRenderProcessGone',
      const StandardMessageCodec(),
    ).setMessageHandler((Object? message) async {
      ...
      if (callback != null) {
        callback(
          AndroidWebResourceError._(
            errorCode: WebViewClientConstants.errorWebContentProcessTerminated,
            description:
                'The WebView renderer process went away (didCrash: ${args[1]}).',
    ...
    _webViewClient = android_webview.WebViewClient(
      ...
      onReceivedClientCertRequest:
          (_, _, android_webview.ClientCertRequest request) {
            request.cancel();
          },
      onReceivedSslError:
          (
